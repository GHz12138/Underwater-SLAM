...
namespace ORB_SLAM3
{
    ...
    Settings::Settings(const std::string &configFile, const int &sensor) : bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false)
    {
        ...
        // if(sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO || sensor_ == System::IMU_RGBD || sensor_ == System::IMU_MONOCULAR_DEPTH){
        //     readIMU(fSettings);
        ...
        if (sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO || sensor_ == System::IMU_RGBD || sensor_ == System::IMU_MONOCULAR_DEPTH)
        {
            ...
            if (sensor_ == System::IMU_MONOCULAR_DEPTH)
            {
                readPressure(fSettings);
                cout << "\t-Loaded Pressure gauge calibration" << endl;
            }
        }
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings)
    {
        ...
        if (cameraModel == "PinHole")
        {
            ...
            if ((sensor_ == System::MONOCULAR || sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_MONOCULAR_DEPTH) && vPinHoleDistorsion1_.size() != 0)
            {
    ...
    void Settings::readPressure(cv::FileStorage &fSettings)
    {
        bool found;
        depthnoise_ = readParameter<float>(fSettings, "Pressure.Noise", found);
        pressureFrequency_ = readParameter<float>(fSettings, "Pressure.Frequency", found);

        // cv::Mat cvTbp = readParameter<cv::Mat>(fSettings,"Pressure.T_b_p",found);
        // Tbp_ = Converter::toSophus(cvTbp);
    ...
    ostream &operator<<(std::ostream &output, const Settings &settings)
    {
        ...
        if (settings.sensor_ == System::IMU_MONOCULAR_DEPTH)
        {
            output << "\t-Gyro noise: " << settings.noiseGyro_ << endl;
            output << "\t-Accelerometer noise: " << settings.noiseAcc_ << endl;
            output << "\t-Gyro walk: " << settings.gyroWalk_ << endl;
            output << "\t-Accelerometer walk: " << settings.accWalk_ << endl;
            output << "\t-IMU frequency: " << settings.imuFrequency_ << endl;
            output << "\t-Depth noise: " << settings.depthnoise_ << endl;
            output << "\t-Pressure frequency: " << settings.pressureFrequency_ << endl;
        }
