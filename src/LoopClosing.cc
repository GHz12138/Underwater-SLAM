...
namespace ORB_SLAM3
{
        ...
        while (1)
        {
            ...
            if (CheckNewKeyFrames())
            {
                ...
                if (bFindedRegion)
                {
                    if (mbMergeDetected)
                    {
                        if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO || mpTracker->mSensor == System::IMU_RGBD || mpTracker->mSensor == System::IMU_MONOCULAR_DEPTH) &&
                                                     ...
                                                     std::vector<MapPoint *> &vpMPs, std::vector<MapPoint *> &vpMatchedMPs)
    {
        ...
        if (nNumProjMatches >= nProjMatches)
        {
            ...
            if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_MONOCULAR_DEPTH) && !pCurrentKF->GetMap()->GetIniertialBA2())
                bFixedScale = false;
                                                 ...
                                                 int &nNumCoincidences, std::vector<MapPoint *> &vpMPs, std::vector<MapPoint *> &vpMatchedMPs)
    {
        ...
        for (KeyFrame *pKFi : vpBowCand)
        {
            ...
            if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
            {
                ...
                if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_MONOCULAR_DEPTH) && !mpCurrentKF->GetMap()->GetIniertialBA2())
                    bFixedScale = false;
    ...
    void LoopClosing::MergeLocal2()
    {
        ...
        if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO || mpTracker->mSensor == System::IMU_RGBD || mpTracker->mSensor == System::IMU_MONOCULAR_DEPTH) && !pCurrentMap->GetIniertialBA2())
        {
